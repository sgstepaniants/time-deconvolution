...
from probability import Distribution, remove_small_masses, sigma_int
from numerical import trap_quad
...
def psi_inv_circle_to_real(lmbda_circle, c0_circle, quad_pts, quad_wts):
    ...
    atoms_real = lmbda_circle.atoms[1:] if delta_at_minus_pi else lmbda_circle.atoms
    atoms_real = mobius(atoms_real)
    atom_wts_real = lmbda_circle.atom_wts[1:] if delta_at_minus_pi else lmbda_circle.atom_wts
    atom_wts_real *= math.pi * (atoms_real**2 + 1) 
    density_real = None if lmbda_circle.density is None else lambda x: lmbda_circle.density(mobius(x))
    
    lmbda_real = Distribution(density=density_real, atoms=atoms_real, atom_wts=atom_wts_real, quad_pts=quad_pts, quad_wts=quad_wts, zero_sets=None, full_support=lmbda_circle.full_support, periodic_domain=None)
    
    # make sure sigma_int can be computed on the circle directly
    c0_real = (sigma_int(lmbda_real)-c0_circle)/np.pi
    c1_real = lmbda_circle.atom_wts[0]/2 if delta_at_minus_pi else 0
    
    return lmbda_real, c0_real, c1_real

def psi_real_to_circle(lmbda_real, c0_real, c1_real, n=100):
    ...
    if c1_real != 0:
        ...
        atom_wts_circle = np.insert(atom_wts_circle, 0, 2*c1_real)
    
    density_circle = None if lmbda_real.density is None else lambda x: lmbda_real.density(mobius_inv(x))
    quad_pts, quad_wts = trap_quad(-np.pi, -np.pi+2*np.pi*(n-1)/n, n)
    
    c0_circle = sigma_int(lmbda_real) - np.pi*c0_real
    
    lmbda_circle = Distribution(density=density_circle, atoms=atoms_circle, atom_wts=atom_wts_circle, quad_pts=quad_pts, quad_wts=quad_wts, zero_sets=None, full_support=lmbda_real.full_support, periodic_domain=(-np.pi, np.pi))
...
def B_circle(lmbda, c0, H=None, compute_mu=True, thresh=1e-15, atoms=None):
    assert(np.isreal(c0))
    assert(np.all(lmbda.quad_wts >= 0))
    assert(np.all(lmbda.atom_wts >= 0))
    ...
    if compute_mu:
        ...
        # Compute discrete part of mu
        alpha_disc = atoms if atoms is not None else H.roots(0, -c0)
