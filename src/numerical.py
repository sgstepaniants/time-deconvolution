...
from . import spectral_transforms
from .kernels import exp_kernel, complex_exp_kernel

def trap_quad(a, b, n):
    assert(a < b)
    assert(n > 1)
    quad_pts, quad_wts = trap_quad_gen(np.linspace(a, b, n))
    return quad_pts, quad_wts

def trap_quad_gen(quad_pts):
    n = len(quad_pts)
    assert(n > 1)
    
    vals = np.diff(quad_pts)
    quad_wts = np.zeros(len(quad_pts))
    quad_wts[:-1] += vals/2
    quad_wts[1:] += vals/2
    return quad_pts, quad_wts
